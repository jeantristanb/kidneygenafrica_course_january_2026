...
egfr_to_creatinine <- function(eGFR, age, sex = c("male", "female")) {
  sex <- match.arg(sex)
...
bimfile<-fread('/spaces/jeantristan/Data/1000Geno/hg38/20220422_3202_phased_SNV_INDEL_SV/plink2/all_1000g_maf0.001.bim')
...
allbim$V2[tbsnp]<-paste(allbim$V1[tbsn_],allbim$V4[tbsnp],allbim$V6[tbsnp],allbim$V5[tbsnp],sep=':')
$order<-1:nrow(bimfile)
...
system('~/bin/plink -bfile /spaces/jeantristan/Data/1000Geno/hg38/20220422_3202_phased_SNV_INDEL_SV/plink2/all_1000g_maf0.001   --extract allsnp.snp --make-bed -out ../Data/all.sample')
...
system('~/bin/gcta64  --bfile ../Data/all.sample  --simu-qt  --simu-causal-loci causal.snplist  --simu-hsq 0.3 --simu-rep 3  --out egfr.init')

egfr_pheno<-read.table('egfr.init.phen')[,1:3];names(egfr_pheno)[1:3]<-c('FID','IID','egfr_i')
...
allegfr$egfr[allegfr$Sex==2]<-allegfr$egfr[allegfr$Sex==2] * 1.05
summary(glm(egfr~Superpopulation+Sex,data=allegfr))
...
write.csv(allegfr, file='../Data/pheno.csv',row.names=F)
